...
import {
  ...
  LocaleSettings,
  CalendarProps,
} from "@superdesk/primereact/calendar";
...
function parseFromPrimeReactCalendarFormat(
  value: CalendarProps["value"]
): IDatePicker["value"] | "failed-to-parse" {
...
function parseToPrimeReactCalendarFormat(
  value: IDatePicker["value"]
): CalendarProps["value"] {
  ...
  constructor(props: IDatePicker) {
    ...
    this.hidePopupOnScroll = throttle(() => {
      if (
        ...
        this.instance.panel.classList.contains(
          internalPrimereactClassnames.overlayVisible
        )
      ) {
  ...
  render() {
    ...
    if (this.props.preview) {
      return (
        ...
        <div>
          <span>
            {moment(this.state.value as Date).format(this.props.dateFormat)}
          </span>
        </div>
      );
    }
      ...
      >
        <Calendar
          ...
          ref={(ref) => {
            ...
            if (
              this.props["data-test-id"] != null &&
              refAny?.inputElement != null
            ) {
              refAny.inputElement.setAttribute(
                "data-test-id",
