...
export default class ToastWrapper extends React.PureComponent<{}, State> {
    ...
    render() {
        return Object.keys(this.state).map((position) => {
            ...
            return (
                <div key={position} className={'sd-toast__container sd-toast__container--' + pos}>
                    {(pos === 'top' ? toasts : [...toasts].reverse()).map((toast: IMessageOptions) => {
                        return (
                            <ToastMessage
                                position={pos}
                                type={toast.type}
                                icon={toast.icon}
                                closeElement={this.requestClose}
