...
class GridItemMedia extends React.PureComponent {
    render() {
        return <div className="sd-grid-item__media">{this.props.children}</div>;
    }
}

class GridItemContent extends React.PureComponent {
    render() {
        return <div className="sd-grid-item__content">{this.props.children}</div>;
    }
}

class GridItemContentBlock extends React.PureComponent {
    render() {
        return <div className="sd-grid-item__content-block">{this.props.children}</div>;
    }
}

class GridItemFooter extends React.PureComponent {
    render() {
        return <div className="sd-grid-item__footer">{this.props.children}</div>;
    }
}
...
class GridItemTime extends React.PureComponent<IPropsTime> {
    render() {
        return <time title={this.props.time}>{this.props.time}</time>;
    }
}

class GridItemTitle extends React.PureComponent {
    render() {
        return <h4 className="sd-grid-item__title">{this.props.children}</h4>;
    }
}

class GridItemSlug extends React.PureComponent {
    render() {
        return <span className="sd-grid-item__slugline">{this.props.children}</span>;
    }
}

class GridItemText extends React.PureComponent {
    render() {
        return <p className="sd-grid-item--element-grow">{this.props.children}</p>;
    }
}

class GridItemTopActions extends React.PureComponent {
    render() {
        return <div className="sd-grid-item__top-actions">{this.props.children}</div>;
    }
}
class GridItemCheckWrapper extends React.PureComponent {
    render() {
        return <span className="sd-grid-item__checkbox">{this.props.children}</span>;
    }
}
